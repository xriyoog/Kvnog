"""
KVN Killer v6.1 — Ultimate UI Overhaul
- Redesigned all UI elements for maximum premium aesthetic
- Modular section boxes (╭─❲ ❳) with custom emojis
...
# ═══════════════════════════════════════
# UI FORMATTERS — ULTIMATE PREMIUM UI
# ═══════════════════════════════════════
...
def box_title(text: str, width: int = 36) -> str:
    line = "═" * width
...
def format_kill_result(card, mm, yy, cvv, result, bin_info, user_name, elapsed, credits_left):
    ...
    return (
        ...
        f"├» {e_card} 𝗧𝘆𝗽𝗲: <b>{bin_info.type}</b> / <b>{bin_info.sub_type}</b>\n"
        f"╰───────────\n\n"
        f"╭─❲ {status_emoji} 𝗥𝗘𝗦𝗨𝗟𝗧 ❳\n"
        f"├» {status_emoji} 𝗦𝘁𝗮𝘁𝘂𝘀: <b>{status_color}</b>\n"
        f"├» {e_link} 𝗚𝗮𝘁𝗲𝘄𝗮𝘆: <b>{result['response']}</b>\n"
        f"╰───────────\n\n"
        f"╭─❲ {e_clock} 𝗦𝗧𝗔𝗧𝗦 ❳\n"
...
def format_bin_lookup(bin_info: BINInfo) -> str:
    ...
    return (
        ...
        f"╭─❲ {e_tag} 𝗜𝗡𝗙𝗢 ❳\n"
        f"├» {e_tag} 𝗕𝗿𝗮𝗻𝗱: <b>{bin_info.brand.upper()}</b>\n"
        f"├» {e_card} 𝗧𝘆𝗽𝗲: <b>{bin_info.type}</b>\n"
...
def main():
    ...
    logger.info("  Ultimate UI Overhaul · Premium Emojis")
