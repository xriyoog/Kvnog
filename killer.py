"""
KVN Killer v5.7 — Backslash Elimination
- Fixed all f-string backslash errors by pre-calculating variables
...
# ═══════════════════════════════════════
# UI FORMATTERS — PRE-CALCULATED EMOJIS
# ═══════════════════════════════════════
...
def format_plan(user_id: int) -> str:
    ...
    return (
        ...
        f"{sep_mid()}\n"
        f"{e_bolt} <code>/kill</code> ▸ CC|MM|YY|CVV — Kill Card\n"
        f"{sep_mid()}\n"
...
def format_menu(is_owner: bool = False) -> str:
    ...
    base = (
        ...
        f"{sep_mid()}\n"
        f"{e_crown} <code>/plan</code>    ▸ Plan Info\n"
        f"{e_money} <code>/credits</code> ▸ Credits Balance\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{title}\n\n"
        f"👋 𝗪𝗲𝗹𝗰𝗼𝗺𝗲, <b>{name}</b>\n"
        f"{e_heart} 𝗞𝗶𝗹𝗹𝗲𝗿 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 — Authorize.net + Payrix\n"
        f"{e_bolt} 𝗦𝗶𝗻𝗴𝗹𝗲 𝗞𝗶𝗹𝗹\n"
        f"🔍 𝗕𝗜𝗡 𝗟𝗼𝗼𝗸𝘂𝗽 · {e_person} 𝗣𝗿𝗼𝗳𝗶𝗹𝗲\n\n"
        f"{format_menu(is_owner)}\n\n"
        f"{e_warn} Use responsibly."
    )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "nav_menu":
        ...
        await query.edit_message_text(
            f"{title}\n\n"
            f"{e_money} 𝗕𝗮𝗹𝗮𝗻𝗰𝗲 ▸ <code>{credits}</code>\n"
            f"{sep_mini()}",
            parse_mode=ParseMode.HTML, reply_markup=back_keyboard())
...
async def id_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{title}\n\n"
        f"{e_person} 𝗬𝗼𝘂𝗿 𝗜𝗗 ▸ <code>{u.id}</code>\n"
